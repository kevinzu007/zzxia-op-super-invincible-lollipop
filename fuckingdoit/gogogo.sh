...
NEED_PRIVILEGES='build&deploy'                   #-- 运行此程序需要的权限，如果需要多个权限，则用【&】分隔
...
F_HELP()
{
    ...
    用法:
        $0  -h|--help
        $0  -l|--list
        $0  [{-c|--category dockerfile|java|node|<自定义>}]  [-D|--debug-port]  [{-b|--branch <代码分支>}]  [{-I|--image-pre-name <镜像前置名称>}]  [{-e|--email <邮件地址>}]  [-s|--skiptest]  [-f|--force]  [-v|--verbose]  [{-V|--release-version <版本号>}]  [-G|--gray]  [<项目1>  <项目2> ... <项目n> ... <项目名称正则匹配>]
    参数规范：
        无包围符号 ：-a                : 必选【选项】
                   ：val               : 必选【参数值】
...
echo "造 浪 者：${MY_USER_XINGMING}" | tee -a ${GOGOGO_BUILD_AND_RELEASE_HISTORY_CURRENT_FILE}
echo "造浪账号：${MY_USER_NAME}@${USER_INFO_FROM}" | tee -a ${GOGOGO_BUILD_AND_RELEASE_HISTORY_CURRENT_FILE}
