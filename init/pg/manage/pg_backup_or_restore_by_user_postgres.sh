...
F_HELP()
{
    ...
    用法:
        ...
        $0  [-r|--restore]  {PATH/TO/FILENAME}  {数据库名}     #--- 恢复，文件须为gzip格式
    参数说明：
        \$0   : 代表脚本本身
        []   : 代表是必选项
        <>   : 代表是可选项
...
F_BACKUP()
{
    FV_BACKUP_TO_PATH_AND_FILENAME=$1
    FV_DB_NAME=$2
    # 备份文件名是否合法
    if ! [[ ${FV_BACKUP_TO_PATH_AND_FILENAME} =~ .*[a-zA-Z0-9]+$ ]]; then
        echo -e "\n猪猪侠警告：备份文件名不合法，必须符合 .*[a-zA-Z0-9]+$ 正则要求，请检查\n"
        exit 1
    fi
    # 目录权限检查
    N=$(echo ${FV_BACKUP_TO_PATH_AND_FILENAME} | grep -o '/' | wc -l)
    if [[ $N -ne 0 ]]; then
        FV_BACKUP_TO_PATH=${FV_BACKUP_TO_PATH_AND_FILENAME%/*}
        if [ -d "${FV_BACKUP_TO_PATH}" ]; then
            touch ${FV_BACKUP_TO_PATH}/test_write >/dev/null 2>&1
            if [ $? != 0 ]; then
                echo -e "\n猪猪侠警告：目录【${FV_BACKUP_TO_PATH}】写入测试失败，请检查\n"
                return 1
            fi
            rm -f ${FV_BACKUP_TO_PATH}/test_write
        else
            echo -e "\n猪猪侠警告：目录【${FV_BACKUP_TO_PATH}】不存在，请检查\n"
            return 1
        fi
    else
        touch ./test_write >/dev/null 2>&1
        if [ $? != 0 ]; then
            echo -e "\n猪猪侠警告：目录【./】写入测试失败，请检查\n"
            return 1
        fi
    ...
    # backup
    ${PGSQL_CMD_BASE}/pg_dump ${FV_DB_NAME} | gzip > ${FV_BACKUP_TO_PATH_AND_FILENAME}.gz
    if [ $? = 0 ]; then
        return 0
    else
        return 1
...
do
    case "$1" in
        ...
        -b|--backup)
            ...
            #
            F_BACKUP ${BACKUP_TO_PATH_AND_FILENAME} ${DB_NAME}
            if [ $? = 0 ]; then
                echo -e "\n猪猪侠警告：数据库【${DB_NAME}】备份成功，备份文件为【${BACKUP_TO_PATH_AND_FILENAME}.gz】\n"
                # send msg
                TIME_END=`date +%Y-%m-%dT%H:%M:%S`
                TIME_COST=`F_TimeDiff "${TIME_START}" "${TIME_END}"`
                ${DINGDING_SEND_SH}  "【Info:PG备份:${RUN_ENV}】"  "数据库${DB_NAME}备份成功！ ${TIME_COST}"
                exit
            else
                ...
                TIME_END=`date +%Y-%m-%dT%H:%M:%S`
                TIME_COST=`F_TimeDiff "${TIME_START}" "${TIME_END}"`
                ${DINGDING_SEND_SH}  "【Info:PG备份:${RUN_ENV}】"  "数据库${DB_NAME}备份失败！ ${TIME_COST}"
                exit 1
            fi
            ;;
        -r|--restore)
            ...
            if [ $? = 0 ]; then
                ...
                TIME_END=`date +%Y-%m-%dT%H:%M:%S`
                TIME_COST=`F_TimeDiff "${TIME_START}" "${TIME_END}"`
                ${DINGDING_SEND_SH}  "【Info:PG还原:${RUN_ENV}】"  "数据库${DB_NAME}还原成功！ ${TIME_COST}"
                exit
            else
                ...
                TIME_END=`date +%Y-%m-%dT%H:%M:%S`
                TIME_COST=`F_TimeDiff "${TIME_START}" "${TIME_END}"`
                ${DINGDING_SEND_SH}  "【Info:PG还原:${RUN_ENV}】"  "数据库${DB_NAME}还原失败！ ${TIME_COST}"
                exit 1
            fi
            ;;
        --)
