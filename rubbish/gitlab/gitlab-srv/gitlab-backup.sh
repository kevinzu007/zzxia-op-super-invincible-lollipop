...
if [ $? != 0 ]; then
    ...
    ${DINGDING_SEND_SH}  "【Error:备份:gitlab】" "gitlab备份：gitlab:backup:create备份出错，请检查！"
    exit 1
fi
...
if [[ -z ${THIS_BACKUP_FILE} ]]; then
    ...
    ${DINGDING_SEND_SH}  "【Error:备份:gitlab】" "gitlab备份：gitlab:backup:create备份出错
，文件为空，请检查！"
    exit 2
fi
...
if [ $? != 0 ]; then
    ...
    ${DINGDING_SEND_SH}  "【Error:备份:gitlab】"  "gitlab备份：ossfs没有挂载，请检查！"
    exit
fi
...
if [ $? != 0 ]; then
    ...
    ${DINGDING_SEND_SH}  "【Error:备份:gitlab】"  "gitlab备份：备份文件拷贝不成功，请检查！"
    exit 3
fi
...
# 每周一发通知
if [ `date +%w` = 1 ]; then
    ${DINGDING_SEND_SH}  "【Info:备份:gitlab】"  "gitlab备份：成功！ ${TIME_COST}"
fi
