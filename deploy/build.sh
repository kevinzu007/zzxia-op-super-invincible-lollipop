...
PROJECT_LIST_RETRY_FILE="${SH_PATH}/${PROJECT_LIST_FILE}.retry"
PROJECT_LIST_FILE_TMP=${PROJECT_LIST_FILE_TMP:-"${LOG_HOME}/${SH_NAME}-${PROJECT_LIST_FILE}.tmp"}
...
DOCKER_BUILD()
{
    ...
    case "${BUILD_METHOD}" in
        docker_*)
            ...
            #
            # build
            docker ${DOCKER_BUILD_OPT} -t ${DOCKER_IMAGE_NAME}  ./  2>&1 | tee ${BUILD_LOG_file}
...
# 输出结果
BUILD_SUCCESS_COUNT=`cat ${BUILD_OK_LIST_FILE} | grep -o 'Build 成功' | wc -l`
BUILD_NOCHANGE_COUNT=`cat ${BUILD_OK_LIST_FILE} | grep -o '无更新' | wc -l`
